...
Этот boilerplate предназначен для быстрого развертывания Laravel-проекта с архитектурой **PHP-FPM 8.5 + Nginx 1.29 (TCP порт 9000)**, где база данных и Redis — **внешние** (не входят в Docker Compose).
...
Nginx проксирует PHP-запросы в PHP-FPM через TCP (порт 9000) по имени сервиса Docker-сети. БД и Redis — внешние сервисы, подключаемые через переменные окружения. Это позволяет использовать managed-сервисы или выделенные серверы для инфраструктуры.
